...
//Las funciones usadas para todo el programa se encuentran en su forma vectorial
double fX(double X[])
{
  return (-X[1]-X[2]);//funcion de x
}
double fY(double X[])
{
  return (X[0]+(a*X[1]));//funcion de y
}
double fZ(double X[])
{
  return (b+(X[2]*(X[0]-c)));//funcion de z
}